...
public final class NumberConvert {
     ...
     * @return
     */
    public static String toHexStrWithAddZero(long num, int strLength) {
        String str = Long.toHexString(num).toUpperCase(Locale.CHINA);
        if (num < 0) {
            int len = str.length();
            if (len < strLength) {
 ...
 /*   public static String getFcsOrHcs(String hcsStr) {
   ...
   /* public static byte[] toBytes(String hcsStr) {
    ...
    /**
     * 获取16进制格式字符串的字节数。 为奇数则自动加1
     * @param hexStr 16进制格式字符串
     * @return int  字符串对应字节数
     */
    public static int getStringByteSize(String hexStr) {
        if (null == hexStr) {
            return 0;
        }
