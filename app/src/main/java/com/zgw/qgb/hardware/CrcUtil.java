...
public class CrcUtil {
     ...
     */
    static int[] fcstab = new int[]{0, 0x1189, 0x2312, 0x329b, 0x4624,
            ...
            0x2c6a, 0x1ef1, 0xf78};
    //Initial   FCS   value
    static char PPPINITFCS16 = 0xffff;
    //Good   final   FCS   value
    static char PPPGOODFCS16 = 0xf0b8;
     ...
     * @return
     */
    public static char pppfcs16(char fcs, byte[] cp, int len) {
        int index = 0;
        while (len-- != 0) {
            fcs = (char) ((fcs >> 8) ^ fcstab[(fcs ^ cp[index]) & 0xff]);
            index++;
        }
        return fcs;
    }
     ...
     * 电能表698协议对应的校验码
     *
     * @param cp  //校验字节数组（校验该字节数组需要在原有的数组后面多加两个0x00,0x00）
     * @param len //检验码之前的字节数组长度
     * @return //检验码
     */
    public static String tryfcs16(byte[] cp, int len) {
        ...
        cp[len] = (byte) (trialfcs & 0x00ff);
        cp[len + 1] = (byte) ((trialfcs >> 8) & 0x00ff);

        byte[] c = {(byte) (trialfcs & 0x00ff)};
        byte[] c1 = {(byte) ((trialfcs >> 8) & 0x00ff)};
        /*   check   on   input   */
        trialfcs = pppfcs16(PPPINITFCS16, cp, len + 2);
        if (trialfcs == PPPGOODFCS16)
            return byteToHexString(c) + byteToHexString(c1);
        return null;
    }
    /**
     * 将byte数组转为字符串
    ...
    public static void main(String[] args) {
        //68 17 00 41 05 88 88 88 88 88 88 10 E3 1E 05 01 00 40 01 02 00 00 ED 03

        byte[] b = new byte[]{(byte) 0x17, (byte) 0x00, (byte) 0x41,
                (byte) 0x05, (byte) 0x88, (byte) 0x88, (byte) 0x88,
                (byte) 0x88, (byte) 0x88, (byte) 0x88, (byte) 0x10,
                (byte) 0xE3, (byte) 0x1E, (byte) 0x05, (byte) 0x01,
                (byte) 0x00, (byte) 0x40, (byte) 0x01, (byte) 0x02,
                (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00};
        //68 17 00 43 45 AA AA AA AA AA AA 00 5B 4F 05 01 00 40 02 02 00 00 20 26 16
        byte[] b1 = new byte[]{(byte) 0x17, (byte) 0x00, (byte) 0x43,
                (byte) 0x45, (byte) 0xAA, (byte) 0xAA, (byte) 0xAA,
                (byte) 0xAA, (byte) 0xAA, (byte) 0xAA, (byte) 0x00,
                (byte) 0x5B, (byte) 0x4F, (byte) 0x05, (byte) 0x01,
                (byte) 0x00, (byte) 0x40, (byte) 0x02, (byte) 0x02,
                (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00};

        byte cp[] = {(byte) 0x03, (byte) 0xC0, (byte) 0x21, (byte) 0x01,
                (byte) 0x02, (byte) 0x00, (byte) 0x09, (byte) 0x03,
                (byte) 0x05, (byte) 0xC0, (byte) 0x23, (byte) 0x80,
                (byte) 0x00, (byte) 0x00};
        // hcs
        String ab = tryfcs16(cp, 12);
        System.out.println(ab);

        // fcs
        String bb = tryfcs16(b1, 21);
